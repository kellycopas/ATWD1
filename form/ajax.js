...
function loadCurrPut() {
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState == 4 && this.status == 200) {
			document.getElementById("put_response").innerHTML = this.responseText;
		}
	};
...
function loadCurrDel() {
	...
	xhttp.onreadystatechange = function () {
		...
		if (this.readyState == 4 && this.status == 200) {
			document.getElementById("delete_response").innerHTML = this.responseText;
		}
	};
	xhttp.open("GET", "currDelete.php?code=", true);
	xhttp.send();
}